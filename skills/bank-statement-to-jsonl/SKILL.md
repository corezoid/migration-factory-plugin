...
description: Turn a bank statement into JSONL, one object per transaction, without reading the statement — sample a single page, work out from the x-coordinates which column each number sits in, declare a ColumnSpec, write a thin parser on top of scripts/statement_lib.py, stream the whole file through it, and prove the result against the totals the statement prints about itself. Handles pdf, xlsx, csv and any text export, files of hundreds of pages, us (1,163.14) and eu (1.238,92) number formats, two-column and single-signed layouts. Use when the user hands over a bank statement, an account extract, a card statement or a transaction export and asks to parse it, convert it, turn it into jsonl or json or rows, pull the transactions out of it, or write a parser for it. Triggers on "распарси выписку", "выписка банка в jsonl", "сделай парсер выписки", "разбери банковскую выписку", "вытащи транзакции из выписки", "конвертируй выписку в json", "выписка из pdf в jsonl", "parse this bank statement", "bank statement to jsonl", "extract the transactions from this statement", "write a parser for this statement", "/bank-statement-to-jsonl".
---

...
description: Load a bank statement into a Digital Twin layer end to end — start a parallel agent parsing every row to JSONL, meanwhile read only the statement's header to work out which bank issued it and which client it was issued for, put the bank on the layer as the company and resolve the client against the client form rather than against the canvas, then collect the parser and post every transaction onto the client's accounts with post_statement. Handles statements of any size: the header is all the main run ever reads, the rows are somebody else's job. Use when the user hands over a bank statement and asks to load it into the graph, the twin, the DTO or Simulator, to record its turnover on a client, or to do both the graph and the transactions. Triggers on "залей выписку в граф", "загрузи выписку в DTO", "выписку в граф и транзакции", "проведи выписку по клиенту", "заведи банк и клиента из выписки", "разнеси выписку по счетам", "load this bank statement into the twin", "import the statement into the graph and post its transactions", "book this statement against the client", "/bank-statement-to-dto".
---
